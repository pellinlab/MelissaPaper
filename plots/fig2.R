...
meanStat = function(x) unlist(purrr::map(x, ~ purrr::map_dbl(.x, ~ mean(.x, na.rm = T))))
sumStat = function(x) purrr::map(x, ~ purrr::map_dbl(.x, sum))
...
for(h in 1:length(idType)) {
  ...
  p = ggplot2 :: ggplot(db)
  p = p + ggplot2 :: scale_x_continuous(breaks = log2(c(1, 2, 4, 8, 16, 32, 64)), labels = c(1, 2, 4, 8, 16, 32, 64), minor_breaks = NULL)
  p = p + ggplot2 :: scale_y_continuous(breaks = breaksy, minor_breaks = NULL)
  p = p + ggplot2 :: labs(x = "effect size h (log2 scale)", 
               y = "<span style = 'color:#0072B2;'>PPV</span>", 
               title = titlesPlot[h], 
               linetype = idLegend)
  p = p + ggplot2 :: theme(axis.title.y = ggtext::element_textbox_simple(width = NULL, orientation = "left-rotated"),
  ...
  p = p + ggplot2 :: geom_line(ggplot2 :: aes(x = x, y = ppv, linetype = s), color = "#0072B2")
  p = p + ggplot2 :: geom_point(ggplot2 :: aes(x = x, y = ppv), color = "#0072B2")
  p = p + ggplot2 :: guides(linetype = ggplot2 :: guide_legend(override.aes = list(colour = "black")))
  ggplot2 :: ggsave(paste0(mainFolder, "/figures/f2/sim_ppv_", idType[h], ".pdf"), width = 5.0, height = 4.0, units = "in")
  ...
  p = ggplot2 :: ggplot(db)
  for(i in 1:nrow(db)) p = p + ggplot2 :: annotate("segment", x = db$x[i], xend = db$x[i], y = 0, yend = db$fwt[i], colour = adjustcolor(db$cl[i], alpha = .2), linewidth = 2.5)
  for(i in 1:nrow(db)) p = p + ggplot2 :: annotate("segment", x = db$x[i], xend = db$x[i], y = 0, yend = db$cov[i], colour = db$cl[i], linewidth = 2.5)
  p = p + ggplot2 :: scale_x_continuous(breaks = c(1,2,3,4,5), labels = c("5kb", "10kb", "20kb", "40kb", "80kb"), minor_breaks = NULL)
  p = p + ggplot2 :: theme(panel.background = ggplot2 :: element_rect(fill = NA),
  ...
  p = p + ggplot2 :: labs(x = "length target gene", y = "coverage")
  
  ggplot2 :: ggsave(paste0(mainFolder, "/figures/f2/sim_cov_", idType[h], ".pdf"), width = 5.0, height = 4.0, units = "in")
}

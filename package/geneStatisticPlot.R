setGeneTable = function(fileGeneList, removeDuplicates = TRUE, removeDots = TRUE, alphabeticalOrder = FALSE) {
  geneTable = data.table :: as.data.table(read.table(fileGeneList, header = TRUE))
...
setGenomeInfo = function() {
  ...
  start   = c(1, cumsum(as.numeric(head(length, -1))))
  bounds  = c(1, cumsum(as.numeric(length)))
...
genesStatisticPlot = function(fileGeneList, fileResults, idSamples, plotHighest = Inf, thresholdPvalue = 1.0, namePlottedGenes = character(), nameSelectedGenes = character(), typePlot = c("segments", "dots")[1], lineWidth = 0.5, transparency = 1, maxLineLength = Inf, inBasePairs = FALSE) {
  ...
  chrBreaks = setGenomeInfo()$boundsChr
  ...
  tablePlot = tablePlot[abs(tablePlot$upHor - tablePlot$upper) > 1e-03]
  ...
  if(typePlot == "segments") {
    ...
    p = p + ggplot2 :: scale_y_continuous(breaks = x_major_breaks, minor_breaks = x_minor_breaks, labels = idSamples)
  } 
  if(typePlot == "dots") {
    p = p + ggplot2 :: geom_point(mapping = ggplot2::aes(y = geneIndex, x = upper, size = zScore, colour = chrColor), alpha = transparency, shape = 1)
    p = p + ggplot2 :: scale_x_continuous(breaks = x_major_breaks, minor_breaks = NULL, labels = idSamples)
  } 
  #if(typePlot == "path") p = p + ggplot2 :: geom_path(mapping = ggplot2::aes(y = geneIndex, x = lower))
  p = p + ggplot2 :: scale_color_manual(values = unlist(colourChromosomes))
  
  #print(plotted_gene_int)
  #print(labelPlottedGenes)
  ...
  p = p + ggplot2 :: scale_x_continuous(breaks = chrBreaks, labels = NULL, minor_breaks = NULL, expand = c(0,0), limits = range(chrBreaks))
  ...
  p = p + ggplot2::theme(#axis.title.y = ggplot2::element_text(size = 7), 
                         ...
                         panel.grid.major.x = list(ggplot2::element_blank(), ggplot2::element_line(colour = "grey90"))[[2]],

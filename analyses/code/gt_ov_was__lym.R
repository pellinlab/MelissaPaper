...
cv = data.frame(time = c(rep(c(22L, 48L, 78L) - 0L, each = 3),
                         rep(c(12L, 36L, 48L, 60L) - 0L, each = 3),
                         rep(c(13L, 36L, 43L, 55L) - 0L, each = 3),
                         rep(c(12L, 30L, 48L) - 0L, each = 3)),
                type = rep(c("B", "T", "NK"), times = sum(c(3, 4, 4, 3))),
                pati = rep(c("W2", "W4", "W5", "W7"), times = c(9, 12, 12, 9)))
cv = cv[-36,]; row.names(cv) = 1:nrow(cv) # missing NK time 0 / 12
...
StoreResults(integrSiteObject, result, nameFileResult = "/analyses/results/gt_ov_was__lym.csv", storeTable = F)
